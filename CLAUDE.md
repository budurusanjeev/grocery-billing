...
- **Scan** (`src/app/scan.tsx` + `src/lib/gemini.ts`): photo → Gemini `gemini-2.5-flash` free tier → JSON items → fuzzy-matched against catalog → **confirmation screen with checkboxes** (never auto-add; handwriting parsing WILL make mistakes). Needs `EXPO_PUBLIC_GEMINI_KEY` in `.env` (free key from aistudio.google.com; `.env` is gitignored, `.env.example` documents it). `EXPO_PUBLIC_` vars are bundled into the client — acceptable for a single-shopkeeper tool, NOT safe for public distribution; that would need a small proxy backend.
