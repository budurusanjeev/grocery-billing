...
- **Scan** (`src/app/scan.tsx` + `src/lib/gemini.ts`): photo → **our backend proxy** `POST https://school-bus-backend-2.onrender.com/api/v1/grocery/scan` (implemented in the sibling `school-bus-backend` repo) → Gemini `gemini-2.5-flash` server-side → JSON items → fuzzy-matched against catalog → **confirmation screen with checkboxes** (never auto-add; handwriting parsing WILL make mistakes). The Gemini key lives ONLY on the server (`GEMINI_API_KEY` env var on the Render backend service) — never in this client. This proxy design exists because Google's newer service-account-bound Gemini keys cannot be website-restricted, so a key bundled into the public site would be stealable; do not move the key back into the client. The backend is on Render's free tier and sleeps when idle — the first scan of the day can take ~1 minute (the loading text warns about this).
