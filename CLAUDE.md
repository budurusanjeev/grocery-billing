...
- **CI** (`.github/workflows/ci.yml`): on every push/PR, runs typecheck, `npm test`, and `expo export -p web` (this last one has caught real deploy-breaking issues before, e.g. an npm peer-dependency conflict that silently broke Render's build for several commits — see `.npmrc`). No secrets needed; the web export step relies on the `EXPO_PUBLIC_*` fallback defaults already baked into the code.
